...
import { MOCK_AUDITORIA_RAD } from '@/lib/radiologia';
...
const ACCIONES = [
  ...
  'Medición en imagen',
  'Firma de informe',
  'Recepción de estudio urgente',
];

export default function AuditoriaRadPage() {
  const [filtroAccion, setFiltroAccion] = useState('Todas');
  const [filtroFecha,  setFiltroFecha]  = useState('');

  const entries = MOCK_AUDITORIA_RAD.filter(e => {
    const matchAccion = filtroAccion === 'Todas' || e.accion === filtroAccion;
    const matchFecha  = !filtroFecha || e.fechaHora.startsWith(
      filtroFecha.split('-').reverse().join('/')
    );
    return matchAccion && matchFecha;
  });

  const filtrosActivos = filtroAccion !== 'Todas' || filtroFecha;
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-6 py-6 space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-bold text-gray-900">Bitácora de Auditoría — Imágenes</h1>
          <p className="text-xs text-gray-500">Dr. Ricardo Mendoza · Cumplimiento Ley 30024 (RENHICE)</p>
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-3 items-end">
          <div>
            ...
            <select value={filtroAccion} onChange={e => setFiltroAccion(e.target.value)}
              className="px-3 py-2 border border-gray-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white min-w-[240px]">
          ...
          {filtrosActivos && (
            <button
              onClick={() => { setFiltroAccion('Todas'); setFiltroFecha(''); }}
              ...
              {entries.map(e => (
                <tr
                  ...
                  className={`hover:bg-gray-50/50 transition-colors ${
                    e.accion.includes('urgente') || e.accion.includes('Firma') ? 'bg-blue-50/30' : ''
                  }`}
                >
                  <td className="px-5 py-3 font-mono text-[11px] text-gray-500 whitespace-nowrap">{e.fechaHora}</td>
                  <td className="px-5 py-3">
                    <span className={`font-semibold ${e.accion.includes('Firma') ? 'text-green-700' : e.accion.includes('urgente') ? 'text-red-700' : 'text-gray-700'}`}>
                      {e.accion}
                    </span>
                  </td>
                  <td className="px-5 py-3">
                  ...
                  <td className="px-5 py-3 text-gray-500 hidden lg:table-cell max-w-[260px] truncate">{e.detalle}</td>
                </tr>
              ))}
              {entries.length === 0 && (
                <tr>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4 flex items-start gap-3">
        ...
        <p className="text-xs text-blue-700">
          ...
          Las imágenes DICOM y los informes radiológicos quedan registrados con fecha, hora, usuario e IP de origen.
        </p>
      </div>
    </div>
  );
