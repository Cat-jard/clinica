...
import { MOCK_AUDITORIA_LAB } from '@/lib/laboratorio';
...
export default function AuditoriaLabPage() {
  ...
  const entries = MOCK_AUDITORIA_LAB.filter(e => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-bold text-gray-900">Bitácora de Auditoría — Laboratorio</h1>
          <p className="text-xs text-gray-500">María Torres · Cumplimiento Ley 30024 (RENHICE)</p>
        </div>
      </div>
              ...
              {entries.map(e => (
                <tr
                  ...
                  className={`hover:bg-gray-50/50 transition-colors ${
                    e.accion.includes('crítico') ? 'bg-red-50/30' : ''
                  }`}
                >
                  <td className="px-5 py-3 font-mono text-[11px] text-gray-500 whitespace-nowrap">{e.fechaHora}</td>
                  <td className="px-5 py-3">
                    <span className={`font-semibold ${e.accion.includes('crítico') || e.accion.includes('Notificación') ? 'text-red-700' : 'text-gray-700'}`}>
                      {e.accion}
                    </span>
                  </td>
                  <td className="px-5 py-3">
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4 flex items-start gap-3">
        ...
        <p className="text-xs text-blue-700">
          ...
          Cada acción realizada en el módulo de laboratorio queda registrada con fecha, hora, usuario e IP de origen.
        </p>
      </div>
    </div>
  );
